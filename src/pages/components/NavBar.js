...
const NavBar = () => {
    ...
    return <div id={`navbar`} className={`w-full h-20 flex md:hidden items-center justify-between navbar ps-20 py-2`}>
        ...
        <div onClick={() => {
            router.push("/search");
        }} id={`search-bar`} className={`w-full h-[49px] relative px-5 mt-2`}>
            <input type={"text"}
                   className={`w-full h-full bg-white py-2  focus:border-lightDark outline-none rounded-full px-2 ps-10`}
            >
            </input>
